...
def create_missing(X, missing_ratio, seed=0):
    """
    ...
    for i in range(X.size(0)):
        for j in range(X.size(1)):
            for k in range(X.size(2)):
                missing_idx = list(state.choice(all_idx, size=int(all_idx.shape[0] * missing_ratio), replace=False))
                X[i, j, k][missing_idx] = 0
                missing_idxs.append(missing_idx)

    return X, missing_idxs

def read_data(dataset, missing_ratio):
    """
    read data from path
    """
    root = './data/'
    ...
    if dataset=='sp500':
        ...
        root+="natops/preprocess/"
        class_list = [str(b'1.0') + ".pickle", str(b'2.0') + ".pickle", str(b'3.0') + ".pickle",
                      str(b'4.0') + ".pickle", str(b'5.0') + ".pickle", str(b'6.0') + ".pickle"]

    elif dataset=='natops2':
        root+="natops-h/"
    ...
    for file in class_list:
        if dataset in ['kor-stock', 'nasdaq', 'natops2']:
            temp_list = []
    ...
    test_X_miss, missing_idx = create_missing(test_X.clone(), missing_ratio)

    trn_idx, val_idx, test_idx = split_data(test_Y, trn_ratio=0.03)
    test_Y = torch.stack([test_Y]).type(torch.float32)

    print(f"\n[Data Description]\n"
          f"Size of source domain: {trn_X.size()}\n"
          f"Size of target domain: {test_X.size()}\n"
          f"Number of non-zero elements: {n_non_zero}\n")

    return trn_X, test_X, test_X_miss, test_Y[0], time_length, max_length, missing_idx, trn_idx, val_idx, test_idx